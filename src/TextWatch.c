...
static int date_mode      = DATE_MODE_OFF;
...
{
  text_layer_set_font(textlayer, date_font);
  text_layer_set_text_color(textlayer, GColorWhite);
...
void init() {
  ...
  window_set_background_color(window, GColorBlack );
    ...
    date_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ORBITRON_BOLD_16));	
...
// seconds layer
	  // 3rd layers
  line4.currentLayer = text_layer_create(GRect(0, 88, 128, 20));
  line4.nextLayer = text_layer_create(GRect(0, 88, 128, 20));
  ...
  layer_add_child(window_layer, (Layer*)line4.currentLayer);
...
void deinit() {
  ...
  layer_destroy((Layer*)line4.currentLayer);
